...
# VeilKey VaultCenter Installer — macOS
#
# Usage:
#   curl -sf http://your-gitlab/veilkey/veilkey-vaultcenter/-/raw/main/install/server-macos.sh | bash
#
# Environment:
...
SERVICE_LABEL="com.veilkey.vaultcenter"
ADDR="${VEILKEY_ADDR:-:10180}"

echo "=== VeilKey VaultCenter Install (macOS) ==="
echo ""
...
# --- Build server ---
build_server() {
    echo "Building veilkey-vaultcenter..."
    cd "$INSTALL_DIR"
    mkdir -p "${INSTALL_DIR}/bin"
    CGO_ENABLED=1 go build -ldflags="-s -w" -o "${INSTALL_DIR}/bin/veilkey-vaultcenter" ./cmd/main.go
    echo "  Built: ${INSTALL_DIR}/bin/veilkey-vaultcenter"
}
...
setup_init() {
    ...
    echo "$password" | VEILKEY_DB_PATH="${DATA_DIR}/veilkey.db" "${INSTALL_DIR}/bin/veilkey-vaultcenter" --init
    echo ""
}
...
<dict>
    ...
    <array>
        <string>${INSTALL_DIR}/bin/veilkey-vaultcenter</string>
    </array>
    ...
    <string>/tmp/veilkey-vaultcenter.log</string>
...
verify() {
    ...
    if echo "$resp" | grep -q '"locked"'; then
        ...
        echo "  Check: cat /tmp/veilkey-vaultcenter.log"
        exit 1
    fi
}
...
echo ""
echo "=== VeilKey VaultCenter installed ==="
echo ""
echo "  Binary:    ${INSTALL_DIR}/bin/veilkey-vaultcenter"
echo "  Data:      ${DATA_DIR}/"
echo "  Salt:      ${DATA_DIR}/salt"
echo "  Logs:      /tmp/veilkey-vaultcenter.log"
