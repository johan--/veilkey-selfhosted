...
func TestInstallRuntimeConfigRoundTrip(t *testing.T) {
	...
	patch := httptest.NewRequest(http.MethodPatch, "/api/install/runtime-config", strings.NewReader(`{
		...
		"target_vmid":"220",
		"host_companion":true,
		"public_base_url":"https://vaultcenter.example.internal",
		...
		"vaultcenter_url":"https://vaultcenter.example.internal",
	...
	if payload.TargetType != "lxc-allinone" || payload.TargetVMID != "220" || !payload.HostCompanion || payload.InstallProfile != "proxmox-lxc-allinone" || payload.PublicBaseURL != "https://vaultcenter.example.internal" {
