...
func TestRootServesDashboardWhenUnlocked(t *testing.T) {
	...
	body := w.Body.String()
	if !strings.Contains(body, "VeilKey VaultCenter") {
...
func TestAdminMockupPreviewRoutesServeHTML(t *testing.T) {
	...
	for _, tt := range tests {
		...
		body := w.Body.String()
		if !strings.Contains(body, "VeilKey VaultCenter") || !strings.Contains(body, "Operations Console (Variation 9)") || !strings.Contains(body, "<div id=\"app\"></div>") {
