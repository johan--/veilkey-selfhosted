...
func (s *Server) handleFederatedRotate(w http.ResponseWriter, r *http.Request) {
	...
	client := &http.Client{Timeout: s.timeouts.Deploy}
	...
	for i := range children {
		...
		if err := s.db.UpdateChildDEK(child.NodeID, encChildDEK, childNonce, newVersion); err != nil {
			results = append(results, rotateResult{
				NodeID: child.NodeID, Label: child.Label,
				Status: "partial", OldVersion: child.Version, NewVersion: newVersion,
				SecretsUpdated: rekeyResp.SecretsUpdated,
