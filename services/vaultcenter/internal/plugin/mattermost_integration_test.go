...
func TestMattermostSyncPlugin(t *testing.T) {
	...
	t.Run("validate_ok", func(t *testing.T) {
		r, _ := inst.Validate(ctx, "/opt/mattermost/config/config.json",
			`{"ServiceSettings":{"SiteURL":"http://mm.test"},"SqlSettings":{"DataSource":"postgres://..."}}`)
		if !r.OK { t.Errorf("expected ok: %s", r.Error) }
	})
	t.Run("validate_missing_siteurl", func(t *testing.T) {
		r, _ := inst.Validate(ctx, "/opt/mattermost/config/config.json",
			`{"ServiceSettings":{"SiteURL":""},"SqlSettings":{"DataSource":"postgres://..."}}`)
		if r.OK { t.Error("expected fail") }
	})
	t.Run("validate_bad_json", func(t *testing.T) {
		r, _ := inst.Validate(ctx, "/opt/mattermost/config/config.json", "not json")
		if r.OK { t.Error("expected fail") }
	})
	t.Run("render", func(t *testing.T) {
		r, _ := inst.Render(ctx, "generate_config", map[string]any{
			"site_url": "http://10.50.0.202:8065", "data_source": "postgres://x", "site_name": "Test", "max_users": float64(50),
		})
