...
require_line "VaultCenter public ref grammar는 \`VK:{SCOPE}:{REF}\` / \`VE:{SCOPE}:{KEY}\` 로 고정"
require_line "VaultCenter family/scope 정책 registry는 canonical 조합만 허용"
require_line "VaultCenter token ref storage는 \`ref_canonical\`과 \`ref_family/ref_scope/ref_id\` component를 함께 유지"
require_line "VaultCenter: stale cleanup 이후의 운영 automation 고도화"
