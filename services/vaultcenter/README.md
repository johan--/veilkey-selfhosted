...
`vaultcenter` is the central self-hosted VeilKey control plane.
...
Required environment variables for the VaultCenter process:
...
VaultCenter is specifically responsible for:
...
## Why VaultCenter Exists

VaultCenter solves the following problems:
...
Even though each LocalVault holds its own secrets, VaultCenter decides "who is who", "what state they are in", and "whether re-registration is allowed".
...
- `installer`
  - installs and verifies VaultCenter runtimes
- `localvault`
  - agent runtime reporting into VaultCenter
- `cli`
...
| Command | Description |
|---------|-------------|
| `veilkey-vaultcenter` | Start server (after init) |
| `veilkey-vaultcenter init --root` | Initialize root node |
| `veilkey-vaultcenter init --child` | Initialize child/localvault node (auto-registers with parent) |
...
```bash
CGO_ENABLED=1 go build -ldflags="-s -w" -o veilkey-vaultcenter ./cmd/main.go
```
...
```bash
# Interactive:
veilkey-vaultcenter init --root
# Or via stdin:
echo "your-password" | veilkey-vaultcenter init --root
```
...
veilkey-vaultcenter init --child \
  --parent http://your-vaultcenter:10180 \
--label my-service \
  --install
```
...
-v /opt/veilkey/password:/run/secrets/veilkey_password:ro \
  ...
  -e VEILKEY_PARENT_URL=http://your-vaultcenter:10180 \
...
services:
  veilkey:
    image: veilkey-vaultcenter:latest
    ports:
      - "10180:10180"
    volumes:
      - veilkey-data:/data
...
VaultCenter is not a central plaintext secret store.
...
- **VaultCenter**: central policy, tracked refs, inventory, lifecycle, decrypt/resolve orchestration
...
VaultCenter maintains inventory based on these values and rejects a LocalVault whose version does not match.
...
- **Tracked ref sync**
  - When a LocalVault performs a `TEMP -> LOCAL/EXTERNAL` promotion or other lifecycle change, the VaultCenter tracked ref is updated accordingly
...
- `tests/integration/removed_endpoints_test.go`
  - Verifies that VaultCenter does not serve local `/api/secrets*` or federation secret routes
